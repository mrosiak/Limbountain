...
namespace Verlet
{
    ...
    public class VerletSimulator {
        ...
        List<Node> particles;
        public float freezeZ = float.MinValue;
        ...
        void Solve(Node a, Node b, float rest)
        {
            ...
            a.position -= a.lockInGrip ? Vector3.zero : f * 0.5f * delta;
            b.position += b.lockInGrip ? Vector3.zero : f * 0.5f * delta;
            if (freezeZ > float.MinValue)
            {
                Freeze(a);
                Freeze(b);
            }
        ...
        void Freeze(Node n)
        {
            n.position = new Vector3(n.position.x, n.position.y, freezeZ);
        }
